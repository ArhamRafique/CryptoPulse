...
import com.muhammad.crypto.domain.model.coin_detail.ValueLabel
...
fun LineChart(
    ...
    val textStyle = MaterialTheme.typography.bodyMedium.copy(fontSize = style.labelFontSize)
    ...
    Canvas(
        ...
        val xLabelLineCount = xLabelTextLayoutResults.maxOfOrNull { it.lineCount } ?: 0
        val xLabelLineHeight = if (xLabelLineCount > 0) {
            maxXLabelHeight / xLabelLineCount
        } else 0
        ...
        val labelCountExcludingLastLabel =
            ((labelViewPortHeightPx / (xLabelLineHeight + minLabelSpacingPx))).toInt()
